...
system.time(sim_res <- marker_smtl_cv(ncore = 63,len =500,data = simdata,X = sim_x))
...
system.time(sim_res_scal_fc <- marker_smtl_cv(ncore = 63,len =1500,data = simdata_sc_fc,X = sim_x_fc))
system.time(sim_res_scal_fc1 <- marker_smtl_cv(ncore = 63,len =1500,data = simdata_sc_fc,X = sim_x_fc))

sim_mse<-lapply(sim_res_scal_fc,function(x) return(as.vector(x)))
sim_mse.nerr<-Map(function(x) {
 if(is.numeric(x)){
   x=x
 }else{
   x=NULL
...
sim_mse.nerr<-sim_mse.nerr[!sapply(sim_mse.nerr,is.null)]
sim_mse.df<-do.call(rbind.data.frame, sim_mse.nerr)
names(sim_mse.df)<-1:36
sim_mse.mx<-as.matrix(sim_mse.df)
matplot(x=t(sim_mse.mx[1:50,]),type="l")

sim_mse1<-lapply(sim_res_scal_fc1,function(x) return(as.vector(x)))
sim_mse.nerr1<-Map(function(x) {
 if(is.numeric(x)){
   x=x
 }else{
   x=NULL
...
sim_mse.nerr1<-sim_mse.nerr1[!sapply(sim_mse.nerr1,is.null)]
sim_mse.df1<-do.call(rbind.data.frame, sim_mse.nerr1)
names(sim_mse.df1)<-1:36
sim_mse.mx1<-as.matrix(sim_mse.df1)
matplot(x=t(sim_mse.mx1[1:50,]),type="l")
